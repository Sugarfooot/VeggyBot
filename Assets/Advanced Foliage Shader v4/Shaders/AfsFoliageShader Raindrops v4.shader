...
Shader "AFS/Foliage Shader Raindrops v4" {
	Properties {
		...
		_BumpRain ("Rain Normal", 2D) = "bump" {}
		_MaskRain ("Rain Mask", 2D) = "white" {}
		_RainTexScale ("Rain Tex Scale", Float) = 4	
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputAFSSpecular o) {
			...
			clip(o.Alpha - _Cutoff);
				...
				//	Rain
				// 	Get rain textures
				fixed3 MaskAll = tex2D(_MaskRain, IN.myuv_MainTex.xy * _RainTexScale).rgb;
				float3 Normal = UnpackNormalDXT5nm(tex2D(_BumpRain, IN.myuv_MainTex.xy * _RainTexScale));
				if (_AfsRainamount > 0.0f) {
					//	Calc WorldNormal
					float3 worldNormal = WorldNormalVector (IN, o.Normal);
				 	float Rainamount = saturate(_AfsRainamount * worldNormal.y);
				 	fixed Mask = MaskAll.g;
					...
					o.Normal = lerp(o.Normal,Normal, saturate(wetBlend * 10) );
				}
