...
Shader "Hidden/Amplify Occlusion/Occlusion" {
	...
	CGINCLUDE
		...
		half2 ComputeOcclusion( v2f_img IN, const int directionCount, const int stepCount, const bool cache, const int source )
		{
			...
			float radius;
			if ( cache )
				radius = ( _AO_RadiusToScreen / p.z ) / 4;
			else
				radius = _AO_RadiusToScreen / p.z;

			const float pi = 3.14159265f;
			const float alpha = 2.0 * pi / directionCount;
			float stepSize = radius / ( stepCount + 1 );
			float occlusion = 0;
			...
			for ( int i = 0; i < directionCount; i++ )
			{
				...
				for ( int j = 0; j < stepCount; j++ )
				{
					...
					occlusion += saturate( nv - _AO_Bias ) * saturate( vv * _AO_NegRcpR2 + 1.0 );
				}
			}
			...
			occlusion = saturate( pow( occlusion, _AO_PowExponent ) );
		...
		{
			half occlusion = tex2D( _AO_OcclusionTexture, IN.uv * _AO_Buffer_PadScale.zw ).r;
			half3 tintedOcclusion = lerp( _AO_Levels.rgb, ( 1 ).xxx, occlusion );
			return lerp( ( 1 ).xxxx, half4( tintedOcclusion.rgb, occlusion ), _AO_Levels.a );
		}
