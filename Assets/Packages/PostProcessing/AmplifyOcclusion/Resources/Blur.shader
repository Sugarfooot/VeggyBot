...
Shader "Hidden/Amplify Occlusion/Blur"
{
	...
	CGINCLUDE
		...
		inline half2 CrossBilateralWeight( const half falloff, const half2 r, half2 d, half d0 )
		{
			half2 diff = ( d0 - d ) * _AO_BlurSharpness;
			return exp2( -( r * r ) * falloff - diff * diff );
		}

		inline half4 CrossBilateralWeight( const half falloff, const half4 r, half4 d, half d0 )
		{
			half4 diff = ( d0 - d ) * _AO_BlurSharpness;
			return exp2( -( r * r ) * falloff - diff * diff );
		}
		...
		half4 blur1D_1x( v2f_img IN, float2 deltaUV )
		{
			...
			half2 w1 = CrossBilateralWeight( falloff, ( 1 ).xx, s1.yw, center.y );
		...
		half4 blur1D_2x( v2f_img IN, float2 deltaUV )
		{
			...
			half4 w12 = CrossBilateralWeight( falloff, half4( 1, 1, 2, 2 ), half4( s1.yw, s2.yw ), center.y );
		...
		half4 blur1D_3x( v2f_img IN, float2 deltaUV )
		{
			...
			half4 w12 = CrossBilateralWeight( falloff, half4( 1, 1, 2, 2 ), half4( s1.yw, s2.yw ), center.y );
			half2 w3 = CrossBilateralWeight( falloff, ( 3 ).xx, s3.yw, center.y );
		...
		half4 blur1D_4x( v2f_img IN, float2 deltaUV )
		{
			...
			half4 w12 = CrossBilateralWeight( falloff, half4( 1, 1, 2, 2 ), half4( s1.yw, s2.yw ), center.y );
			half4 w34 = CrossBilateralWeight( falloff, half4( 3, 3, 4, 4 ), half4( s3.yw, s4.yw ), center.y );
